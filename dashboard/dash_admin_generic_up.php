...
<?php
error_reporting(0);
$get_id = $_GET['genericid'];
$id = $_SESSION["id"] = $get_id;  

if (isset($_GET['genericid']))
{
   $sql3 = "SELECT * FROM `med_generic` WHERE `genericid`=$get_id";
   $result=$conn->query($sql3);
   while($row3=$result->fetch_assoc()){
	   $oldname = $row3['generic_name'];
   }
}
...
{
	$updated_gn = $_POST['generic_name'];
	$sql4 = "UPDATE `med_generic` SET `generic_name`='$updated_gn' WHERE `genericid`=$id";
	$result4=$conn->query($sql4);
	unset($_SESSION["id"]);
	header('Location: dash_admin_generic.php');
}
		   ...
		   <div class="mdc-layout-grid">	  
            <div class="mdc-layout-grid__inner">
			  ...
			  <div class="mdc-layout-grid__cell stretch-card mdc-layout-grid__cell--span-6">
                <div class="mdc-card">
				<!-- do code here -->
			<form id="reg-form" action="dash_admin_generic_up.php?genericid=<?php echo $id ?>" method="post" enctype="multipart/form-data">
					
			  <div class="template-demo">
				<div class="mdc-layout-grid__inner">
				  <div class="mdc-layout-grid__cell stretch-card mdc-layout-grid__cell--span-12-desktop">
					 <div class="mdc-text-field mdc-text-field--outlined">
					  <input class="mdc-text-field__input" id="text-field-hero-input" value= <?php echo $oldname; ?> name="generic_name">
					  <div class="mdc-notched-outline">
						<div class="mdc-notched-outline__leading"></div>
						<div class="mdc-notched-outline__notch">
						  <label for="text-field-hero-input" class="mdc-floating-label" >Update Generic Name</label>
						</div>
						<div class="mdc-notched-outline__trailing"></div>
					  </div>
					</div>
                ...
                <div class="mdc-card p-0">
                  ...
                  <div class="table-responsive">
                    <table class="table table-hoverable">
                      <thead>
                        <tr>
                          <th class="text-right">Generic ID</th>
                          <th>Generic Name</th>
                          <th>Edit</th>
                        </tr>
                      </thead>
                      <tbody>
						 <?php
						   $sql2 = "SELECT * FROM `med_generic` WHERE `status` = 1";
						   ...
						   { ?>
							<tr>
							  ...
							  <?php echo "<td align='center'> <a title='Update Generic Name' href=\"dash_admin_generic_up.php?genericid=$row2[genericid]\"onClick=\"return confirm('Are you sure to update this Generic?')\"><i class='material-icons'>create</i></a></td>" ?>
							</tr>
						 <?php }?>
                      </tbody>
                    </table>
