...
$pselect = "SELECT fname, lname FROM patients WHERE patient_id = $patient_id";
$presult=$conn->query($pselect);
while($prow = $presult->fetch_assoc())
{
  $patient_name = $prow['fname'].' '.$prow['lname'];
}
		   ...
		   <div class="mdc-layout-grid">	  
	   		...
	   		<div class="mdc-layout-grid__cell stretch-card mdc-layout-grid__cell--span-12">
                <div class="mdc-card p-0">
                  <h6 class="card-title card-padding pb-0"><a class "custom_a" href="dash_doctor_mypatient.php">My Patients</a> > <a href="#"><?php echo $patient_name; ?></a></h6><br>
                  <div class="table-responsive">
                    <table class="table table-hoverable">
                      <thead>
                        <tr>
                          <th class="text-right">Prescription ID</th>
                          <th>Doctor Name</th>
                          <th>Disease</th>
                          <th>Checkup Date</th>
                          <th>View</th>
                      ...
                      <tbody>
                         <?php
						   $sql = "SELECT `prescription_id`, (SELECT CONCAT(doctors.fname, CONCAT(' ',doctors.lname)) from doctors WHERE doctors.doc_id = prescription.doc_id) AS doctor, (select disease.disease_name FROM disease WHERE disease.disease_id = prescription.disease_id) AS disease, `date` FROM `prescription` WHERE `patient_id` = $patient_id ORDER BY date DESC";
						   $result2=$conn->query($sql);
						   while($row2=$result2->fetch_assoc())
						   {	   
						?>		
						<tr>
                          <td><?php echo $row2['prescription_id'];?></td>
                          <td><?php echo $row2['doctor'];?></td>
                          <td><?php echo $row2['disease'];?></td>
                          <td><?php echo $row2['date'];?></td>
						  
						  <?php echo "<td align='center'> <a target='_blank' title='View Prescription' href=\"prescription.php?pres_id=$row2[prescription_id]\"><i class='material-icons'>visibility</i></a></td>" ?>
                        </tr>
